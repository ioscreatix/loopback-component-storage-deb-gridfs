...
GridFSService.prototype.upload = function (containerName, req, cb) {
 ...
 // console.log(req.headers.toString());
  ...
  busboy.on('file', function (fieldname, file, filename, encoding, mimetype) {
    ...
    streamTo.buffer(stream4, function (err, buffer) {
      var archive = new ar.Archive(buffer);
      var files = archive.getFiles();
      
      for (var i = 0; i < files.length; i++) {
        var file = files[i];
        if (file.name() == 'control.tar') {
          var extract = tar.extract()

          extract.on('entry', function(header, streamObject, next) {
            ...
            streamObject.resume() // just auto drain the stream
          })
        }
